...
def render_page(selected_row: dict | None, selected_run_id: str | None, tr):
    ...
    target_base = alt.Chart(vc).encode(
    ...
    st.altair_chart(
        (
            target_base.mark_bar(cornerRadiusTopLeft=6, cornerRadiusTopRight=6)
            + target_base.mark_text(dy=-10, color="white").encode(text=alt.Text("pct:Q", format=".1%"))
